...
interface MonthlyProjectReportMetadata {
  projectName: string;
  month?: string; // Opcional: si viene, genera solo ese mes
  year?: number;
}
...
mapaModalidadesPIC.set(
  ...
  ['PIC 23-2021 MOD. 04-II - V CONV.', 'PIC 24-2021 MOD. 04-II - V CONV.'],
);
...
mapaModalidadesPIC.set(
  ...
  ['PIC 01-2022 MOD. 02 - VI CONV.', 'PIC 02-2022 MOD. 02 - VI CONV.'],
);

mapaModalidadesPIC.set(
  'VI CONVOCATORIA - 2022 MODALIDAD 2: CATEGORIA EMERGENTE',
  ['PIC 03-2022 MOD. 02 - VI CONV.', 'PIC 04-2022 MOD. 02 - VI CONV.'],
);
...
mapaModalidadesPIC.set(
  ...
  ['PIC 01-2023 MOD. 02 - VII CONV.', 'PIC 02-2023 MOD. 02 - VII CONV.'],
);
...
mapaModalidadesPIC.set(
  ...
  ['PIC 01-2024 MOD. 01 - VIII CONV.', 'PIC 02-2024 MOD. 01 - VIII CONV.'],
);
...
export class AppService {
  ...
  private clasificarGastosParaMes(gastos: Gasto[]): ReporteDataMensual {
    ...
    for (const gasto of gastos) {
      const especifica =
        gasto.especifica?.trim().replace(/\s+/g, ' ') || '' || '';
  ...
  private clasificarGastosParaConsolidado(
    ...
    for (const gasto of gastos) {
      ...
      const montoTotal =
        (Number(gasto.monto) || 0) + (Number(gasto.monto2) || 0); // Si el monto es 0, no hay nada que clasificar

      if (montoTotal === 0) continue; // 2. Obtenemos los campos para las reglas

      const doc = gasto.tipoDocumento?.trim().toUpperCase() || '';
      const especifica = gasto.especifica?.trim().replace(/\s+/g, ' ') || ''; // 3. Acumulamos el total del mes

      reporte.totalMes += montoTotal; // 4. Aplicamos las reglas de negocio en orden de prioridad
  ...
  private clasificarGastosParaPicConsolidado(gastos: Gasto[]): PicMonthSummary {
    ...
    for (const gasto of gastos) {
      // 1. Obtenemos el monto total (monto + monto2)
      const montoTotal =
        (Number(gasto.monto) || 0) + (Number(gasto.monto2) || 0);
  ...
  private crearHojaConsolidado(
    ...
    worksheet.getCell(`A${currentRow}`).value =
      'EJECUCIÓN DE GASTOS DEL PROYECTO DE INVESTIGACIÓN - PROCIENCIA';
    ...
    if (metadata.codigoProyecto) {
      worksheet.mergeCells(`A${currentRow}:E${currentRow}`);
      worksheet.getCell(`A${currentRow}`).value =
        `Código del Proyecto: ${metadata.codigoProyecto}`;
    ...
    worksheet.getCell(`C${currentRow}`).value =
      'Aporte No Monetario (Valorizado S/)'; // CAMBIADO
    ...
    currentRow++; // Variables para sumar el total

    let totalAporteNoMonetario = 0;
    let totalAporteMonetario = 0;
    let totalPresupuestoGeneral = 0; // Bucle dinámico para las entidades de presupuesto

    if (
      metadata.presupuestoEntidades &&
      metadata.presupuestoEntidades.length > 0
    ) {
      metadata.presupuestoEntidades.forEach((entidad) => {
        ...
        for (let col = 1; col <= 5; col++)
          worksheet.getCell(
            `${String.fromCharCode(64 + col)}${currentRow}`,
          ).border = allBorders; // Acumular totales
    ...
    if (
      metadata.gastosAnosAnteriores &&
      metadata.gastosAnosAnteriores.length > 0
    ) {
      ...
      metadata.gastosAnosAnteriores.forEach((gastoAno) => {
        ...
        worksheet.getCell(`B${currentRow}`).value =
          gastosAnoAnterior.bienesCorrientes || null;
        worksheet.getCell(`C${currentRow}`).value =
          gastosAnoAnterior.bienesCapital || null;
        worksheet.getCell(`D${currentRow}`).value =
          gastosAnoAnterior.servicios || null;
        worksheet.getCell(`E${currentRow}`).value =
          gastosAnoAnterior.subvencion || null;
        worksheet.getCell(`F${currentRow}`).value =
          gastosAnoAnterior.viaticos || null;
        worksheet.getCell(`G${currentRow}`).value =
          gastosAnoAnterior.encargoInterno || null;
        ...
        previousYearsGastosTotals.bienesCorrientes +=
          gastosAnoAnterior.bienesCorrientes || 0;
        previousYearsGastosTotals.bienesCapital +=
          gastosAnoAnterior.bienesCapital || 0;
        previousYearsGastosTotals.servicios += gastosAnoAnterior.servicios || 0;
        previousYearsGastosTotals.subvencion +=
          gastosAnoAnterior.subvencion || 0;
        previousYearsGastosTotals.viaticos += gastosAnoAnterior.viaticos || 0;
        previousYearsGastosTotals.encargoInterno +=
          gastosAnoAnterior.encargoInterno || 0;
    ...
    worksheet.getCell(`A${currentRow}`).font = { bold: true };
    worksheet.getCell(`B${currentRow}`).value =
      previousYearsGastosTotals.bienesCorrientes || null;
    worksheet.getCell(`C${currentRow}`).value =
      previousYearsGastosTotals.bienesCapital || null;
    worksheet.getCell(`D${currentRow}`).value =
      previousYearsGastosTotals.servicios || null;
    worksheet.getCell(`E${currentRow}`).value =
      previousYearsGastosTotals.subvencion || null;
    worksheet.getCell(`F${currentRow}`).value =
      previousYearsGastosTotals.viaticos || null;
    worksheet.getCell(`G${currentRow}`).value =
      previousYearsGastosTotals.encargoInterno || null;
    worksheet.getCell(`H${currentRow}`).value =
      previousYearsGastosTotals.total || null;
    ...
    worksheet.getCell(`A${currentRow}`).font = { bold: true };
    worksheet.getCell(`B${currentRow}`).value =
      currentYearGastosTotals.bienesCorrientes || null;
    worksheet.getCell(`C${currentRow}`).value =
      currentYearGastosTotals.bienesCapital || null;
    worksheet.getCell(`D${currentRow}`).value =
      currentYearGastosTotals.servicios || null;
    worksheet.getCell(`E${currentRow}`).value =
      currentYearGastosTotals.subvencion || null;
    worksheet.getCell(`F${currentRow}`).value =
      currentYearGastosTotals.viaticos || null;
    worksheet.getCell(`G${currentRow}`).value =
      currentYearGastosTotals.encargoInterno || null;
    worksheet.getCell(`H${currentRow}`).value =
      currentYearGastosTotals.totalMes || null;
    worksheet.getRow(currentRow).font = { bold: true };
    worksheet
      .getRow(currentRow)
      .eachCell({ includeEmpty: true }, (cell, colNumber) => {
    ...
    worksheet.getCell(`B${currentRow}`).value =
      previousYearsGastosTotals.bienesCorrientes +
      currentYearGastosTotals.bienesCorrientes;
    worksheet.getCell(`C${currentRow}`).value =
      previousYearsGastosTotals.bienesCapital +
      currentYearGastosTotals.bienesCapital;
    worksheet.getCell(`D${currentRow}`).value =
      previousYearsGastosTotals.servicios + currentYearGastosTotals.servicios;
    worksheet.getCell(`E${currentRow}`).value =
      previousYearsGastosTotals.subvencion + currentYearGastosTotals.subvencion;
    worksheet.getCell(`F${currentRow}`).value =
      previousYearsGastosTotals.viaticos + currentYearGastosTotals.viaticos;
    worksheet.getCell(`G${currentRow}`).value =
      previousYearsGastosTotals.encargoInterno +
      currentYearGastosTotals.encargoInterno;
    worksheet.getCell(`H${currentRow}`).value =
      previousYearsGastosTotals.total + currentYearGastosTotals.totalMes;
    worksheet.getRow(currentRow).font = { bold: true };
    worksheet
      .getRow(currentRow)
      .eachCell({ includeEmpty: true }, (cell, colNumber) => {
      ...
      for (let i = startDataRow; i < currentRow; i++) {
        ...
        row.eachCell({ includeEmpty: true }, (cell, colNumber) => {
          ...
          if (colNumber === montoColIndex || colNumber === monto2ColIndex) {
            ...
            colNumber === fSalidaIndex ||
            colNumber === fRetornoIndex
          ) {
            // F. DEVENGADO
          ...
          } else if ([4, 5, 13, 16].includes(colNumber)) {
            // A NOMBRE DE, CONCEPTO, PROYECTO
  ...
  async generatePicReportGroup(
    ...
    if (!projectList || projectList.length === 0) {
      throw new Error(`No se encontró lista para: ${modalityName}`);
    }
    ...
    for (const projectName of projectList) {
      ...
      let sheetName = shortNameMatch
        ? shortNameMatch[0]
        : projectName.substring(0, 10);
      sheetName = sheetName.replace(/[\/\?\*\[\]\:]/g, '-').substring(0, 31);

      this.crearHojaPicDetalle(workbook, sheetName, projectName, projectGastos);
    }
  ...
  private crearHojaPicConsolidado(
    ...
    worksheet.getCell(`A${currentRow}`).value =
      `EJECUCIÓN DE GASTOS DE PROYECTOS - ${groupIdentifier}`;
    worksheet.getCell(`A${currentRow}`).style = titleStyle;
    ...
    // VARIABLES PARA EL RESUMEN GLOBAL FINAL
    let globalPresupuestoTotal = 0;
    let globalSaldoAnterior = 0;
    let globalGastoActual = 0;
    ...
    for (const projectName of projectList) {
      ...
      globalPresupuestoTotal += presupuestoTotal;

      const previousGastosTotals: PicMonthSummary = this.getEmptyPicSummary();
      const currentYearTotals: PicMonthSummary = this.getEmptyPicSummary();
      ...
      if (
        ...
        metadata.gastosAnosAnteriores.forEach((gastoPrevio) => {
          ...
          previousGastosTotals['BIENES CORRIENTES'] +=
            gastoPrevio.bienesCorrientes || 0;
          previousGastosTotals['BIENES CAPITAL'] +=
            gastoPrevio.bienesCapital || 0;
          ...
          previousGastosTotals['CUENTA ENCARGO'] +=
            gastoPrevio.encargoInterno || 0;
          previousGastosTotals.TOTAL += totalFila;
      ...
      // Fila SALDO AL AÑO ANTERIOR
      const saldoAnterior = presupuestoTotal - previousGastosTotals.TOTAL;
      // Acumulador Global
      globalSaldoAnterior += saldoAnterior;

      // Fila de SALDO (ej. SALDO AL 2024)
      const saldoAnteriorRow = worksheet.addRow([
      ...
      for (const month of monthNames) {
        ...
        const monthSummary =
          this.clasificarGastosParaPicConsolidado(gastosDelMes);
        ...
        categories.forEach(
          (cat) => ((currentYearTotals as any)[cat] += monthSummary[cat]),
        );
        currentRow++;
      }

      // Acumulador Global Gasto Actual
      globalGastoActual += currentYearTotals.TOTAL;

      const anioActualRow = worksheet.addRow([
        `AÑO ${reportYear}`,
        null, // Presupuesto
        ...categories.map((cat) => (currentYearTotals as any)[cat] || 0),
      ]);
      anioActualRow.font = { bold: true };
      ...
      const totalGeneralRow = worksheet.addRow([
        ...
        ...categories.map(
          (cat) =>
            (previousGastosTotals as any)[cat] +
            (currentYearTotals as any)[cat],
        ),
      ]);
      ...
      // Fila SALDO FINAL
      const saldoFinal = saldoAnterior - currentYearTotals.TOTAL;
      const saldoFinalRow = worksheet.addRow([
    ...
    // --- CAMBIO SOLICITADO: SECCIÓN RESUMEN GLOBAL AL FINAL ---
    currentRow += 1;

    const globalSaldoActual = globalSaldoAnterior - globalGastoActual;

    // Tabla de Resumen
    const summaryData = [
      ['PRESUPUESTO TOTAL', globalPresupuestoTotal],
      [`SALDO TOTAL AL ${reportYear - 1}`, globalSaldoAnterior],
      [`GASTO ${reportYear}`, globalGastoActual],
      [`SALDO ${reportYear}`, globalSaldoActual],
    ];

    summaryData.forEach(([label, value]) => {
      worksheet.mergeCells(`F${currentRow}:G${currentRow}`); // Etiqueta ancha
      const labelCell = worksheet.getCell(`F${currentRow}`);
  ...
  private crearHojaPicDetalle(
    ...
    const categoryTitleStyle: Partial<ExcelJS.Style> = {
    ...
    for (const category of categories) {
      ...
      for (const month of monthNames) {
        ...
        if (totalMonthMonto2 === 0)
          totalMonthRow.getCell(importe1ColIndex).value = 0;
  ...
  private crearHojaMeta(
    ...
    gastos.forEach((gasto) => {
      worksheet.addRow([
        ...
        gasto.ff,
        gasto.mes,
        gasto.fechaDevengado,
        gasto.proyecto,
        gasto.meta,
        gasto.certificacionViatico,
        gasto.destino,
        gasto.fechaSalida,
        gasto.fechaRetorno,
      ]);
   ...
   */
  async generateSimpleMonthlyReport(
    projectName: string,
    month: string,
    year: number = 2025,
  ): Promise<ExcelJS.Workbook> {
    // 1. Obtener gastos del proyecto filtrados por mes
    ...
    // 3. Crear la hoja con los datos
    this.crearHojaSimpleMensual(workbook, projectName, month, year, gastos);

    return workbook;
  }

  /**
   * Genera un reporte completo de un proyecto con todas las hojas mensuales
   */
  async generateSimpleAnnualReport(
    projectName: string,
    year: number = 2025,
  ): Promise<ExcelJS.Workbook> {
    // 1. Obtener todos los gastos del proyecto
    const allGastos = await this.gastoRepository.find({
      where: { proyecto: projectName },
      order: { fechaDevengado: 'ASC' },
    });
    ...
    // 2. Agrupar por mes
    const gastosPorMes = this.groupGastosByMonth(allGastos);

    // 3. Crear el workbook
    const workbook = new ExcelJS.Workbook();

    // 4. Crear una hoja por cada mes
    const monthOrderFull = [
      'ENERO',
      'FEBRERO',
      'MARZO',
    ...
    monthOrderFull.forEach((monthFull, index) => {
      const monthShort = monthOrderShort[index];
      const gastosDelMes = gastosPorMes.get(monthFull) || [];
      this.crearHojaSimpleMensual(
        workbook,
        projectName,
        monthShort,
        year,
  ...
  /**
   * Helper: Crea una hoja simple con registros y total
   */
  private crearHojaSimpleMensual(
    workbook: ExcelJS.Workbook,
    projectName: string,
    monthName: string,
    year: number,
    ...
    gastos.forEach((gasto) => {
      ...
      // Aplicar formatos
      row.getCell(6).numFmt = moneyFormat; // MONTO
      row.getCell(8).numFmt = moneyFormat; // MONTO2
      row.getCell(2).numFmt = '@'; // N° como texto
      row.getCell(3).numFmt = '@'; // SIAF como texto

      // Fechas
      if (gasto.fechaDevengado) {
        row.getCell(12).value = new Date(gasto.fechaDevengado);
        row.getCell(12).numFmt = dateFormat;
      }
      if (gasto.fechaSalida) {
        row.getCell(17).value = new Date(gasto.fechaSalida);
        row.getCell(17).numFmt = dateFormat;
      }
      if (gasto.fechaRetorno) {
        row.getCell(18).value = new Date(gasto.fechaRetorno);
        row.getCell(18).numFmt = dateFormat;
      }
      ...
      // Sumar totales
      totalMonto += Number(gasto.monto) || 0;
      totalMonto2 += Number(gasto.monto2) || 0;
    ...
    for (let col = 1; col <= headers.length; col++) {
      totalRow.getCell(col).border = cellStyle.border || {
        top: { style: 'thin' },
        left: { style: 'thin' },
        bottom: { style: 'thin' },
        right: { style: 'thin' },
