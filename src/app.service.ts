  ...
  private crearHojaDeReportePorMes(workbook: ExcelJS.Workbook, sheetName: string, data: ReporteDataMensual, projectName: string): void {
    ...
    const renderSection = (title: string, gastos: Gasto[]) => {
      ...
      gastos.forEach(gasto => {
        ...
        addedRow.getCell(6).numFmt = '"S/" #,##0.00';
      ...
      totalRow.getCell(6).style = { ...totalRowStyle, numFmt: '"S/" #,##0.00', alignment: { horizontal: 'left' } };
      currentRow += 2;
    ...
    grandTotalRow.getCell(6).style = { ...totalRowStyle, numFmt: '"S/" #,##0.00', alignment: { horizontal: 'left' } };
    ...
    for (let i = 1; i <= 14; i++) {
        ...
        if (column) {
            ...
            column.eachCell({ includeEmpty: true }, cell => {
                maxTextLength = Math.max(maxTextLength, (cell.text || '').length);
            });
            column.width = maxTextLength < 12 ? 12 : maxTextLength + 2;
        }
    }
    */
  }
